...
export const dijkstra = (
  ...
  while (untraversedTiles.length > 0) {
    ...
    untraversedTiles.sort((a, b) => a.distance as number - (b.distance as number)); // Sort the queue by distance
    ...
    if (currentTile) {
      ...
      for (let i = 0; i < neighbors.length; i += 1) {
        ...
        if (currentTile.distance as number + 1 < ( neighbors[i].distance as number)) {
