...
const retrieveHeuristicCost = (currentTile: Tiletype, endTile: Tiletype) => {
  ...
  const r = Math.abs(currentTile.row as number - (endTile.row as number )); // Calculate the absolute difference in rows between the current tile and the end tile
  const c = Math.abs(currentTile.col as number - (endTile.col as number) ); // Calculate the absolute difference in columns between the current tile and the end tile
  return manhattanDistance * (r + c); // Return the Manhattan distance (sum of row and column differences)
};

export const initHeuristicCost = (grid: Grid_Types, endTile: Tiletype) => {
